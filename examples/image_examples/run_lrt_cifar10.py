"""CIFAR-10 classification with BayesianNetworkLRT (grayscale).

Images are converted to grayscale (32×32 = 1024 pixels) before flattening.
This keeps the input tractable for a fully-connected LBBNN and lets
`plot_model_vision_image` infer the 32×32 shape automatically
(int(sqrt(1025)) = 32, p*p = 1024).

Run from the image_examples/ directory:
    python run_lrt_cifar10.py
...
from LBBNN import BayesianNetworkLRT, train_epoch, validate, plotting, weight_matrices
...
SEED = 42
DATA_DIR = Path("data")
RESULTS_DIR = Path("results/lrt_cifar10")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

N_CLASSES = 10
IMG_PIXELS = 32 * 32   # 1024 (grayscale)
...
# Data helpers
# ============================================================
def _to_grayscale(X_nhwc: torch.Tensor) -> torch.Tensor:
    """(N,H,W,3) float → (N,H*W) grayscale via luminance weights."""
    gray = 0.299 * X_nhwc[:, :, :, 0] + 0.587 * X_nhwc[:, :, :, 1] + 0.114 * X_nhwc[:, :, :, 2]
    return gray.view(gray.shape[0], -1)


def _load_split(train: bool) -> tuple[torch.Tensor, torch.Tensor, np.ndarray]:
    """Return (X_with_bias, y_long, X_gray_pixels_np)."""
    ds = datasets.CIFAR10(root=str(DATA_DIR), train=train, download=True)
    X_raw = torch.tensor(ds.data).float() / 255.0  # (N, 32, 32, 3)
    X_flat = _to_grayscale(X_raw)                   # (N, 1024)
    y = torch.tensor(ds.targets).long()
    X_bias = torch.cat([torch.ones(len(X_flat), 1), X_flat], dim=1)  # (N, 1025)
...
def main() -> None:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"[LRT CIFAR-10] Epoch {epoch:03d} | "
    ...
    print(f"[LRT CIFAR-10] Test accuracy: {test_acc:.4f}")
    ...
    save_json(
        {"model_type": "BayesianNetworkLRT", "dataset": "CIFAR-10 (grayscale)",
         "seed": SEED, "device": str(DEVICE), "p": P, "dim": DIM,
         "hidden_layers": HIDDEN_LAYERS, "epochs": EPOCHS,
         "batch_size": BATCH_SIZE, "learning_rate": LR, "test_accuracy": test_acc},
    ...
    print(f"[LRT CIFAR-10] Results saved to: {RESULTS_DIR.resolve()}")
