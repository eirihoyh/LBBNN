"""MNIST digit classification with BayesianNetworkLRT.

Run from the image_examples/ directory:
    python run_lrt_mnist.py
...
# Allow importing _common.py from the parent examples/ directory
...
SEED = 42
DATA_DIR = Path("data")
RESULTS_DIR = Path("results/lrt_mnist")
...
def main() -> None:
    ...
    print(f"[LRT MNIST] Test accuracy: {test_acc:.4f}")
    ...
    save_json(
        {"model_type": "BayesianNetworkLRT", "dataset": "MNIST",
    ...
    print(f"[LRT MNIST] Results saved to: {RESULTS_DIR.resolve()}")
