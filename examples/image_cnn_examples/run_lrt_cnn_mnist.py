"""MNIST digit classification with BayesianNetworkCNNLRT.
...
from LBBNN import (BayesianNetworkCNNLRT, train_epoch, validate,
...
SEED = 42
DATA_DIR = Path("data")
RESULTS_DIR = Path("results/lrt_cnn_mnist")
...
def main() -> None:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"[LRT-CNN MNIST] Epoch {epoch:03d} | "
    ...
    print(f"[LRT-CNN MNIST] Test accuracy: {test_acc:.4f}")
    ...
    save_json(
        {"model_type": "BayesianNetworkCNNLRT", "dataset": "MNIST",
    ...
    # Note: full inspection utilities (average_path_length, input_inclusion_prob)
    # assume the first layer width equals p, which does not hold for CNNs.
    # We use only the safe subset here.
    model.eval()
    ...
    print(f"[LRT-CNN MNIST] Results saved to: {RESULTS_DIR.resolve()}")
