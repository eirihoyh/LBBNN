...
$\text{I}\text{\_}n$ is the inputs, where $n=0$ inidcates the bias node, $n=1$ is $x_1$ and so on. $\text{H}l\text{\_}n$ is the $n^{th}$ hidden node in layer $l$, while $\text{Output}\text{\_}0$ is the output node. It can be noted here that neither $\text{I}\text{\_}3$ nor $\text{I}\text{\_}4$ is included in the MPM structure, meaning the model has successfully ignored them. Additionally, the first three hidden layers are not included in the MPM, leaving a much simpler structure. The initial structure had 1625 weigths, while the final MPM used 32. 
